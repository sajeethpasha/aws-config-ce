...
#     bucket_name = 'config-bucket-381492161162'
#     prefix = 'AWSLogs/381492161162/Config/us-east-1/2024/5/'
...
#                 'configurationItemCaptureTime': item['configurationItemCaptureTime'][:10],  # Extracting date part
...
#     s3 = boto3.client('s3')
#     bucket = 'randomcheckforconfig'
#     key = 'costs/cost-report/cost-report/year=2024/month=5/cost-report-00001.snappy.parquet'
...
#         entry['line_item_usage_start_date'] = entry['line_item_usage_start_date'][:10]  # Extracting date part
#         entry['line_item_usage_end_date'] = entry['line_item_usage_end_date'][:10]
...
#         config_df,
#         cost_df,
#         left_on=['resourceId', 'configurationItemCaptureTime'],
#         right_on=['line_item_resource_id', 'line_item_usage_start_date']
#     )
    
#     return merged_df

# def save_to_csv_by_date(merged_df):
#     # Group by the date part of the configurationItemCaptureTime
#     grouped = merged_df.groupby('configurationItemCaptureTime')
...
#     save_to_csv_by_date(merged_df)
#     print("Merged Data saved to CSV files by date.")
...
#         config_df,
#         cost_df,
#         how='left',  # Perform a left merge to ensure all config data is retained
...
def get_cost(event, context):
    s3 = boto3.client('s3')
    bucket = 'lambdacodeforcost'
    key = '2024-05-19.csv.gz'
    response = s3.get_object(Bucket=bucket, Key=key)
    gzipped_content = response['Body'].read()
...
#     config_df = pd.DataFrame(config_data)
#     grouped = config_df.groupby('configurationItemCaptureDate')
...
def lambda_handler(event, context):
    try:
        ...
        # Save the individual config and cost data to separate CSV files
        save_config_to_csv_by_date(config_data)
        save_cost_to_csv_by_date(cost_data)
